...
export function Button({
  ...
  const styles = {
    primary: "bg-brand-700 text-white hover:bg-brand-600 disabled:bg-accent-500",
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full rounded-lg border border-black/15 bg-white px-3 py-2 text-sm outline-none focus:border-brand-600 focus:ring-1 focus:ring-brand-600 dark:border-white/15 dark:bg-neutral-800 ${props.className ?? ""}`}
    />
  );
}
